...
class DynamicApprovalRule(models.Model):
    ...
    def _send_notification(self, user, approval_request):
        ...
        if approval_request.res_model in ["product.template", "product.product"]:
            if approval_request.res_model == "product.template":
            ...
            body = Markup(
                f"Reminder: Please approve Product <b>{pr.name}</b>.<br/>"
            ...
            if approval_request.approval_rule_id.notify_email and user.email:
                mail_vals = {
                    "subject": f"Reminder: Approval Needed for Product - {pr.name}",
    ...
    def _do_post_action(self, request, line, role):
        ...
        if request.res_model in ["product.template", "product.product"]:
            if role == "primary":
            ...
            if action == "reallocate":
                if request.res_model == "product.template":
                    product = self.env["product.template"].sudo().browse(request.res_id)
                else:
                    product = self.env["product.product"].sudo().browse(request.res_id)
                ...
                product.message_post(body=Markup(msg), message_type="notification")
        return res
